...
const LiveLog = dynamic(() => import("./LiveLog"), { ssr: false });
...
export default function Board() {
  ...
  async function openLog(sessionId) {
    setLogFor(sessionId); // LiveLog fetches its own token and streams via WS
  }
    ...
    <div style={{ padding: "20px 24px" }} onClick={() => runningOpen && setRunningOpen(false)}>
      ...
      {logFor && (
        <Modal onClose={() => setLogFor(null)} wide>
          <LiveLog sessionId={logFor} onClose={() => setLogFor(null)} />
        </Modal>
      )}
